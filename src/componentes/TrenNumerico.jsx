...
const NIVELES = [
  { titulo: 'Ordena los números de mayor a menor', id: 1, vagones: [5, 2, 8, 3], solucion: [8, 5, 3, 2] },
  { titulo: 'Ordena los números del 1 al 5', id: 2, vagones: [3, 1, 5, 2, 4], solucion: [1, 2, 3, 4, 5] },
  { titulo: 'Ordena los números de 5 en 5', id: 3, vagones: [20, 5, 15, 10, 25], solucion: [5, 10, 15, 20, 25] },
  { titulo: 'Ordena los números de 10 en 10', id: 4, vagones: [20, 10, 30, 50, 40], solucion: [10, 20, 30, 40, 50] },
  { titulo: 'Ordena los números de menor a mayor', id: 5, vagones: [12, 9, 15, 7, 11], solucion: [7, 9, 11, 12, 15] },
];
...
function Vagon({ numero, estaEnTren, isDragging }) {
  ...
  const claseVagon = `tren-vagon ${estaEnTren ? 'tren-vagon-colocado' : 'tren-vagon-disponible'}`;
  ...
  return (
    <div
      ...
      {...listeners}
      {...attributes}
      className={claseVagon}
    >
      <span className="tren-vagon-texto">{numero}</span>
    </div>
  );
}
...
function ZonaTren({ children, isError, haGanado }) {
  ...
  const clases = `tren-zona-vias ${isOver ? 'tren-zona-over' : ''} ${isError ? 'tren-anim-shake' : ''} ${
    haGanado ? 'tren-anim-ganado' : ''
  }`;
  
  return (
    <div ref={setNodeRef} className={clases}>
      <div className="tren-locomotora"></div>
      {children}
    </div>
  );
}
...
export default function TrenNumerico() {
  ...
  const renderJuego = () => (
    <div className="tren-body-wrapper">
      <DndContext
      ...
      >
        <div className="tren-contenedor-principal">
          <div className="tren-cabecera">
            <h2>
          ...
          {haGanadoNivel && <div className="tren-mensaje-victoria">¡Chuuuu-chuuu!🎉</div>}

          <div className="tren-zona-disponibles">
            <h3>Vagones disponibles:</h3>
            <div className="tren-lista-vagones">
        ...
        <DragOverlay>
          {activeId && (
            <div className="tren-vagon tren-vagon-flotante">
              <span className="tren-vagon-texto">{activeId}</span>
            </div>
          )}
        </DragOverlay>
      </DndContext>
  ...
  if (gameStatus === 'menu') {
    return (
      <div className="tren-body-wrapper">
        <div className="tren-menu-card">
          <h1 className="tren-titulo-juego">El Tren Numérico</h1>
          <div className="tren-img-locomotora-menu"></div>
          <p>¡Ayuda a ordenar los vagones del tren!</p>
          <button className="tren-boton-accion" onClick={() => cargarNivel(0)}>
            ¡Jugar!
          </button>
        </div>
      </div>
  ...
  if (gameStatus === 'levelComplete') {
    ...
    return (
      <div className="tren-body-wrapper">
        <div className="tren-overlay-nivel">
          <div className="tren-modal-contenido">
            <h2>¡Nivel {NIVELES[currentLevelIndex].id} Completado!</h2>
            <div className="tren-estrellas-anim">
              <span>⭐</span>
              <span>⭐</span>
              <span>⭐</span>
            </div>
            <p>¡Excelente trabajo!</p>
            <button className="tren-boton-accion" onClick={buttonAction}>
              {buttonText}
            </button>
          </div>
        </div>
  ...
  if (gameStatus === 'allComplete') {
    return (
      <div className="tren-body-wrapper">
        <div className="tren-menu-card">
          <h1 className="tren-titulo-juego">¡Felicidades!</h1>
          <div className="tren-img-locomotora-menu"></div>
